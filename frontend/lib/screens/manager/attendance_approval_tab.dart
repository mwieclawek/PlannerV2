...
class AttendanceApprovalTab extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<AttendanceApprovalTab> createState() =>
      _AttendanceApprovalTabState();
}
...
class _AttendanceApprovalTabState extends ConsumerState<AttendanceApprovalTab>
  ...
  void _exportPdf() async {
    ...
    final url = api.getAttendanceExportUrl(
      _startDate,
      _endDate,
      status: _statusFilter,
      token: token,
  ...
  Future<void> _confirmAttendance(String id) async {
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Obecność potwierdzona'),
  ...
  Future<void> _rejectAttendance(String id) async {
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Obecność odrzucona'),
                        ...
                        FilledButton(
                          onPressed: () {
                            if (selectedUserId == null) {
                              ScaffoldMessenger.of(context).showSnackBar(
                                const SnackBar(
                                  content: Text('Wybierz pracownika'),
                                ),
                              );
                              return;
                            }
  ...
  Future<void> _saveManualAttendance(
    String userId,
    DateTime date,
    String checkIn,
    String checkOut,
      ...
      children: [
        Container(
          ...
          child: TabBar(
            ...
            tabs: const [
              Tab(text: 'Obecności', icon: Icon(Icons.fact_check_outlined)),
              Tab(text: 'Urlopy', icon: Icon(Icons.beach_access_outlined)),
            ],
          ),
        ),
        Expanded(
                                  ...
                                  child: ListTile(
                                    ...
                                    subtitle: Column(
                                      ...
                                      children: [
                                        ...
                                        Text(
                                          'Godz: ${attendance['check_in']} - ${attendance['check_out']}',
                                        ),
                                        if (!(attendance['was_scheduled']
                                                as bool) &&
                                            isPending)
                                          Text(

...
class _TeamTabState extends ConsumerState<TeamTab> {
                          ...
                          isLoading
                              ...
                              : () async {
                                ...
                                if (firstName.isEmpty) {
                                  ScaffoldMessenger.of(context).showSnackBar(
                                    const SnackBar(
                                      content: Text('Imię jest wymagane'),
                                    ),
                                  );
                                  return;
                                }
                                ...
                                try {
                                  ...
                                  if (context.mounted) {
                                    ...
                                    ScaffoldMessenger.of(context).showSnackBar(
                                      const SnackBar(
                                        content: Text('Dane zaktualizowane'),
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(const SnackBar(content: Text('Role zapisane')));
      }
  ...
  void _showPreferencesDialog(TeamMember user) {
    final hoursController = TextEditingController(
      text: user.targetHoursPerMonth?.toString() ?? '',
    );
    final shiftsController = TextEditingController(
      text: user.targetShiftsPerMonth?.toString() ?? '',
    );
  ...
  Future<void> _savePreferences(
    ...
    try {
      final api = ref.read(apiServiceProvider);
      // If the user cleared both fields, we want to erase them on the backend
      final clearTargets = targetHours == null && targetShifts == null;
      await api.updateUser(
        userId,
        targetHoursPerMonth: targetHours,
        targetShiftsPerMonth: targetShifts,
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Preferencje zapisane'),
            backgroundColor: Colors.green,
          ),
        );
      }
                  ...
                  actions: [
                    ...
                    FilledButton(
                      onPressed: () async {
                        ...
                        if (newPass.length < 6) {
                          setDialogState(
                            () => errorText = 'Hasło musi mieć min. 6 znaków',
                          );
                          return;
                        }
      ...
      content = RefreshIndicator(
        ...
        child: ListView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            final userRoles =
                _roles?.where((r) => user.jobRoleIds.contains(r.id)).toList() ??
                [];
                              ...
                              : () async {
                                ...
                                try {
                                  ...
                                  await api.createUser(
                                    ...
                                    email:
                                        emailController.text.isNotEmpty
                                            ...
                                            : null,
                                    targetHoursPerMonth: int.tryParse(
                                      hoursController.text,
                                    ),
                                    targetShiftsPerMonth: int.tryParse(
                                      shiftsController.text,
                                    ),
                                  );
                                  ...
                                  if (context.mounted) {
                                    ...
                                    ScaffoldMessenger.of(context).showSnackBar(
                                      const SnackBar(
                                        content: Text('Dodano pracownika'),
                                        backgroundColor: Colors.green,
                                      ),
                                    );
                                    _loadData();

...
class _OrderCreationScreenState extends ConsumerState<OrderCreationScreen> {
      ...
      data: (categories) {
        if (categories.isEmpty) {
          return const Scaffold(
            body: Center(child: Text('Brak skonfigurowanych menu kategorii')),
          );
        }
                    ...
                    data:
                        (items) => TabBarView(
                          children:
                              categories.map((cat) {
                                final catItems =
                                    items.where((i) => i.categoryId == cat.id).toList();
                                if (catItems.isEmpty) {
                                  return Center(
                                    child: Text(
                                      'Brak pozycji',
  ...
  Future<void> _submitOrder() async {
    ...
    try {
      ...
      final payload = cartNotifier.toApiPayload();
      await api.createOrderV2(widget.table.id, payload);
